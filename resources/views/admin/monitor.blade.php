...
<td class="px-4 py-4 border-r">
    <button type="button" 
        ...
        data-nomor="{{ $globalCounter - 1 }}"> {{-- Tambahkan atribut data-nomor --}}
        {{ $ind->nama_indikator }}
    </button>
</td>
...
if (buktiArray.length > 0) {
    ...
    if (filteredLogsAdmin.length > 0) {
        filteredLogsAdmin.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).forEach(log => {
            ...
            const formattedDate = date.toLocaleString('id-ID', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
            html += `
                <tr>
                    <td class="px-4 py-2 font-bold text-blue-600 dark:text-blue-400">${log.name}</td>
                    <td class="px-4 py-2 font-semibold text-gray-500 uppercase">${log.role}</td>
 ...
 content.innerHTML = html;
    ...
    saveBtn.onclick = async () => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                const res = await fetch('/penilaian-kriteria/store', { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest' } });
                if (res.ok) {
                    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data penilaian eksternal telah disimpan.', showConfirmButton: false, timer: 1500 })
                    .then(() => window.location.reload());
                } else { throw new Error(); }
            } catch (e) { Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan data.', 'error'); }
        }
    };
