...
<div id="domain-container" class="space-y-6">
    ...
    Kriteria
            ...
            <button @click="openModal='edit-indikator-{{ $i->id_indikator }}'" 
                    class="px-3 py-2 bg-amber-50 text-amber-700 border border-amber-100 rounded-lg text-[11px] font-bold hover:bg-amber-500 hover:text-white transition-all duration-200 active:scale-95">
                Edit
            </button>

            <button @click="openModal='delete-indikator-{{ $i->id_indikator }}'" 
                    class="px-3 py-2 bg-red-50 text-red-700 border border-red-100 rounded-lg text-[11px] font-bold hover:bg-red-600 hover:text-white transition-all duration-200 active:scale-95">
                Hapus
            </button>
        </div>
    </div>
 ...
 <template x-teleport="body">
    <div id="modal-container">
        ...
        @if(View::exists('admin.master.modals.add_domain'))
            @include('admin.master.modals.add_domain')
        @endif

      @foreach($domain as $d)
    @include('admin.master.modals.domain', ['d' => $d])

    <div x-show="openModal === 'add-aspek-{{ $d->id_domain }}'" 
         x-cloak 
         class="fixed inset-0 z-[999] flex items-center justify-center p-4">
        
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" @click="openModal = null"></div>

        <div class="relative bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl shadow-2xl border border-white/20 overflow-hidden"
             @click.stop>
    ...
    @foreach($d->aspek as $a)
        ...
        @foreach($a->indikator as $i)
            @include('admin.master.modals.indikator', ['i' => $i, 'a' => $a])
        @endforeach
    @endforeach
@endforeach
    </div>
