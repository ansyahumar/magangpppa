    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">
                Verifikasi nilai yang telah diinput oleh Asesor Internal.
            </p>
        </div>
    </div>
                                    ...
                                    @foreach($a->indikator as $ind)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            ...
                                            <td class="px-4 py-4">
                                                <button
                                                    type="button"
                                                    class="text-left font-bold text-blue-600 dark:text-blue-400 hover:underline indikator-item"
                                            ...
                                            <td class="px-4 py-4 text-center">
                                                @php
                                                    $indikatorData = data_get($draft, $ind->id_indikator);
                                                    $nilaiVerifikasi = $indikatorData ? $indikatorData->nilai_verifikator_internal : 0;
                                                @endphp

                                                @if($nilaiVerifikasi > 0)
                                                ...
                                                @else
                                                    <span class="text-[10px] font-bold text-amber-500 uppercase tracking-tighter italic">
                                                        Belum Diverif
                                                    </span>
                                                @endif
                                            </td>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-6xl w-full flex flex-col max-h-[90vh] overflow-hidden border dark:border-gray-700">
        ...
        <div class="flex justify-between items-center px-6 py-4 border-b dark:border-gray-700 text-gray-800 dark:text-white bg-gray-50 dark:bg-gray-900/50">
            <div>
                ...
                <p id="modal-subtitle" class="text-xs text-gray-500 mt-1 uppercase tracking-widest font-bold">
                    Panel Kontrol Verifikator
                </p>
            </div>
            <button id="close-modal" class="p-2 bg-gray-200 dark:bg-gray-700 rounded-full dark:text-white font-bold hover:bg-red-500 transition-all">
                ✕
            </button>
        </div>

        <!-- Modal Content -->
        <div id="kriteria-content" class="p-6 overflow-y-auto custom-scrollbar"></div>
        ...
        <div class="px-6 py-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 text-right">
            <button
                type="button"
                id="save-kriteria"
                class="hidden px-8 py-2.5 bg-blue-600 text-white font-black rounded-xl shadow-lg hover:bg-blue-700 transition-all uppercase text-xs tracking-widest"
            >
...
const statusPenilaianGlobal = "{{ $statusPenilaian }}";
const isFinal = statusPenilaianGlobal === 'final'; 
   let isEditMode = false;
    let activeIndikatorId = null;
const isFinalGlobal = statusPenilaianGlobal === 'final';
...
            renderUI(data, nomorUrut, nilaiHistoriTahunLalu);
        } catch (e) {
            console.error(e);
...
function renderUI(data, nomorUrut, nilaiHistori) {
...
    const isFinal = data.status_penilaian === 'final'; 
const isLocked = (data.status_vrifU === 'final' || isFinalGlobal);
    const canEditVerif = (userRole === 'verifikator' && !isLocked);
    if (canEditVerif) {
        saveBtn.classList.remove('hidden');
    } else {
        saveBtn.classList.add('hidden');
        ...
        <table class="min-w-full text-[11px] text-left divide-y divide-gray-200">
            <thead class="bg-gray-50 font-bold text-center text-gray-600">
                <tr>
                    ...
                    <th rowspan="2" class="px-4 py-2 border-b border-r bg-gray-100/50 italic">Tahun ${tahunHistori}</th>
                ...
                <tr class="text-[8px] uppercase">
                    <th class="border-r bg-gray-100/50">Akhir</th> <th class="px-2 py-2 border-r"></th>
...
        const level = (i % 5) + 1;
        const isChecked = parseFloat(levelVerifAktif) === level;
 ...
 html += `
            <tr class="hover:bg-blue-50/30 transition-colors">
                ...
                <td class="text-center border-r bg-gray-50/30">
                    <input type="checkbox" ${chkHistori} disabled class="rounded w-3 h-3">
                </td>

                <td class="text-center border-r">
                    <input type="checkbox" ${parseFloat(levelTarget) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-emerald-500">
                </td>

                <td class="text-center border-r bg-slate-50/50">
                    <input type="checkbox" ${parseFloat(levelAsesorInt) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-slate-500">
                </td>

                <td class="text-center border-r bg-blue-50">
                    <input type="checkbox" class="kriteria-cb w-4 h-4 rounded text-blue-600 cursor-pointer" 
                        ${isChecked ? 'checked' : ''} 
                        ${canEditVerif ? '' : 'disabled'} 
                        data-kriteria="${k.id_kriteria}" value="${level}">
                </td>

                <td class="text-center border-r bg-amber-50/30">
                    <input type="checkbox" ${parseFloat(levelAsesorExt) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-amber-500">
                </td>
                <td class="text-center bg-amber-50/30">
                    <input type="checkbox" ${parseFloat(levelAkhirExt) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-amber-600">
                </td>
            </tr>`;
    });
        ...
        <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200 text-left">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="pencapaian-verif" class="w-full border-gray-200 rounded-lg text-xs" rows="4" 
                        ${canEditVerif ? '' : 'readonly'} 
                        placeholder="Masukkan catatan verifikasi...">${cat.pencapaian_verif_internal || ''}</textarea>
                        </div>
                <div class="space-y-2">
...
            title: 'Simpan Data Verifikasi?',
            text: 'Pastikan kriteria dan catatan sudah sesuai.',
            icon: 'question',
