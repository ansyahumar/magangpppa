...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cat = (data.catatan && data.catatan.length > 0) 
                ? data.catatan[0] 
                : { pencapaian: '', bukti: '[]', pencapaian_verif_internal: '', nama_catatankriteria: '' };
    ...
    <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 mb-6 shadow-sm">
        <table class="min-w-full text-[11px] text-left divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900/80 font-bold text-center text-gray-600 dark:text-gray-300">
                <tr>
                    <th rowspan="3" class="px-4 py-3 border-r dark:border-gray-700 w-12 text-center">Lv</th>
                    <th rowspan="3" class="px-4 py-3 border-r dark:border-gray-700 min-w-[300px] text-left">Kriteria Penilaian</th>
                    <th class="px-4 py-2 border-b border-r dark:border-gray-700 bg-gray-100/50 dark:bg-gray-800 italic">Tahun ${tahunHistori}</th>
                    <th colspan="5" class="px-4 py-2 border-b dark:border-gray-700">Tahun Aktif (${tahunAktif})</th>
                </tr>
                <tr class="text-[9px] uppercase tracking-tighter">
                    <th class="border-r dark:border-gray-700 bg-gray-100/50 dark:bg-gray-800">Nilai Akhir</th>
                    <th class="px-4 py-2 border-r dark:border-gray-700">Target</th>
                    <th colspan="2" class="px-4 py-2 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-black border-r dark:border-gray-700">Internal</th>
                    <th colspan="2" class="px-4 py-2 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 font-black">Eksternal</th>
                </tr>
                <tr class="text-[8px] uppercase">
                    <th class="border-r dark:border-gray-700 bg-gray-100/50 dark:bg-gray-800"></th>
                    <th class="px-2 py-2 border-r dark:border-gray-700">P2</th>
                    <th class="px-2 py-2 border-r dark:border-gray-700">Asesor</th>
                    <th class="px-2 py-2 border-r dark:border-gray-700">Verif</th>
                    <th class="px-2 py-2 border-r dark:border-gray-700">Asesor</th>
                    <th>Akhir</th>
                </tr>
            </thead>
    ...
    kriteriaList.forEach((k, i) => {
        ...
        html += `
            <tr class="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                <td class="text-center font-bold text-gray-400 border-r dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">${level}</td>
                <td class="p-3 leading-relaxed border-r dark:border-gray-700 dark:text-gray-300 text-[11px]">${k.nama_kriteria}</td>
                <td class="text-center border-r dark:border-gray-700 bg-gray-50/30 dark:bg-gray-900/20"><input type="checkbox" ${parseFloat(levelHistori) === level ? 'checked' : ''} disabled class="rounded w-3 h-3"></td>
                <td class="text-center border-r dark:border-gray-700"><input type="checkbox" ${parseFloat(levelTarget) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-emerald-500"></td>
                <td class="text-center border-r dark:border-gray-700 bg-slate-50/50 dark:bg-slate-900/20"><input type="checkbox" ${parseFloat(levelAsesorInt) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-slate-500"></td>
                <td class="text-center border-r dark:border-gray-700 bg-blue-50 dark:bg-blue-900/40">
                ...
                <td class="text-center border-r dark:border-gray-700 bg-amber-50/30 dark:bg-amber-900/10"><input type="checkbox" ${parseFloat(levelAsesorExt) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-amber-500"></td>
                <td class="text-center bg-amber-50/30 dark:bg-amber-900/10"><input type="checkbox" ${parseFloat(levelAkhirExt) === level ? 'checked' : ''} disabled class="rounded w-3 h-3 text-amber-600"></td>
            </tr>`;
    });
        ...
        <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200 text-left">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="space-y-2">
                    ...
                    <div id="display-${cat.id_catatan}" class="flex flex-wrap gap-2">`;
 ...
 html += `<div class="border-t dark:border-gray-700 pt-6 mt-6 text-left">
                <h5 class="text-xs font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em] mb-4 flex items-center">
    ...
    <span class="text-[8px] px-1.5 py-0.5 rounded font-bold uppercase 
        ${
            roleName === 'user' ? 'bg-blue-600 text-white' : 
            roleName === 'p2'   ? 'bg-emerald-600 text-white' : 
            'bg-orange-500 text-white'
        }">
        ${
            roleName === 'user' ? 'ASESOR INTERNAL' : 
            roleName === 'p2'   ? 'P2' : 
            'VERIFIKATOR1'
        }
    </span>
  ...
  saveBtn.onclick = async () => {
        ...
        if (isEditMode && !checked) {
            Swal.fire('Peringatan', 'Silakan pilih salah satu kriteria verifikasi sebelum menyimpan perubahan.', 'warning');
            return;
        }
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                if (res.ok) {
                    Swal.fire('Berhasil', isEditMode ? 'Perubahan data berhasil disimpan.' : 'Data verifikasi berhasil diinput.', 'success')
